...
9. hero-card 用**專屬** 45° 漸層（`linear-gradient(45deg, #F9D42E … #FF8A3D)`，左下更黃），文字改 `#ff5700` 橘字（不再是黑）
...
4. 素材相關收尾：五張手繪圖都已引用並 commit。`public/Assets/` 內還有一張 hash 名 jpg 未使用、未追蹤。`.DS_Store` 已被 `.gitignore` 擋。手繪圖是 jpg 未壓縮，若之後變多可考慮壓一版。
