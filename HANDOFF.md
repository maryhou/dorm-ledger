...
> 給下一個 session / 接手者的交接文件。最後更新：2026-07-11（v1.4）。
...
## 目前狀態：v1.4 已上線 ✅（素材大換血＋物品圖示支援圖片＋preset 擴充）
...
| `public/Assets/` | 裝飾圖：`welcome`／`empty`／`groceries`（jpg 區塊圖）、`outofstock.gif`（售罄動圖）、`Eye`／`Smile`（行內小圖）；`shared_items/*.png` 是物品圖示。從根目錄提供，路徑 `public/Assets/xxx` |
...
4. 素材相關收尾：目前 `public/Assets/` 裡的檔案全部有被引用（使用者已自己清掉沒用到的）。圖片未壓縮，數量變多了（16 個檔案），之後可考慮壓一版。
