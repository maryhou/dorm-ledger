...
### 2026-07-11 這輪（v1.4）：素材大換血＋物品圖示支援圖片（都是使用者逐項點名要的）

1. **裝飾圖全面換新**（使用者自備新素材，舊圖 `Home_empty`／`Record_empty`／`Happy` 已從磁碟與 repo 刪除）：
   - 歡迎頁（沒室友）→ `welcome.jpg`（兩人相擁；曾試 `welcome001.jpg` 三人喝咖啡，不採用已刪）
   - 物品空狀態 🧺 emoji → `groceries.jpg`（原本這格是 emoji，現在也是 `.empty-gif` 200px）
   - 紀錄空狀態 → `empty.jpg`
   - 售罄面板 → `outofstock.gif`（洞裡比讚，動圖）
   - header 副標「帳目乾乾淨淨，感情好好」尾端 💚 移除（純文字；`subGreeting` 仍走 `textContent`）
2. **物品圖示同時支援 emoji 和圖片**：`emoji` 欄位值含 `/` 視為圖片路徑，由 `itemIcon()` 渲染成 `<img class="icon-img">`（1.15em、em 基準跟著字級縮放）；toast 純文字用 `itemIconText()` 自動略過圖片圖示。渲染點：物品卡、紀錄列、使用/補貨面板標題、圖示選擇列、preset 列。
3. **圖片圖示素材放 `public/Assets/shared_items/`**，目前 10 張 png：popsicle／coffeebean／teabag／tissue／tea／lemon／butter／salmon／cabbage／bread，全部加進圖示選擇列尾端（共 30 選項）
4. **內建 preset 調整**：新增「咖啡豆 $15」（coffeebean.png，排咖啡後）、「冰棒 $20」（popsicle.png，排最後）；咖啡 $25→$45、蛋 $15→$10
5. **圖示選擇列改兩排橫捲**（`.emoji-row` 改 grid `grid-auto-flow: column` 兩列，直向優先排序）；室友頭像列共用此樣式，12 隻剛好兩排六欄不用捲。`applyPreset` 加 `scrollIntoView` 讓選中的圖示自動捲到畫面中央
6. sw.js `CACHE` 版本 v8→v9，`ASSETS` 清單同步換成新素材（含 shared_items 全部）

### 2026-07-11 第二輪（v1.5）：編輯功能大補完（都是使用者逐項點名要的）

1. **選取樣式全面改主漸層框**：`.emoji-opt.sel`／`.chip.sel`（及新的 `.avatar-tab.sel`）從黑框改為雙層背景技巧——內層 `--lime-soft` 鋪 padding-box、`--grad` 鋪 border-box、2px 透明 border 露出漸層（border-color 吃不了漸層，只能這樣做）
2. **帳本可改名**：`db.title`（無此欄位 fallback「宿舍小帳本」），header 大標題（`#appTitle` span）與 `document.title` 都跟著變；入口在室友頁
3. **室友頁底部改「管理」settings 卡**（使用者拿 iOS 設定風格截圖當參考）：`.settings-card`/`.settings-row`，一列一動作、左 outline icon 右 chevron（`ICONS.chev` 新增）；「幫帳本改名」+「清空全部資料」（破壞性動作紅字＋trash icon）。之後新增設定項就往這張卡加列
4. **物品可換圖示**：使用面板標題的圖示是按鈕（`.icon-swap` 淡底小方塊示意可點）→「換個圖示」面板（同一套兩排圖示列，現值帶選取框並捲到可視處）→ 點選即存、跳回使用面板。注意：log 只存 itemId，換圖後**歷史紀錄也會顯示新圖**（與價格快照不同），屬預期行為
5. **頭像雙分頁**：加入/編輯室友的頭像列上方有「動物頭像／人物頭像」pill 分頁；動物＝原 12 隻 emoji，人物＝`public/Assets/avatar_image/avatar_01–08.png`（`MEMBER_AVATARS` 常數，**寫死 8 張**，加圖要同步改 app.js 與 sw.js）。兩分頁共用單一選取（`pickAvatar` 跨列清除）；選擇器抽成 `avatarPickerHTML(selected)` 共用
6. **室友可編輯**：點室友列（cursor＋按壓回饋）→「編輯室友」sheet（名字預填、頭像帶現值）→`saveMember`；✕ 刪除鈕加 `event.stopPropagation()` 防誤開
7. `avatarHTML()` 支援圖片頭像（同「含 / 即圖片」慣例，`.avatar-img` 填滿圓框 object-fit:cover）
8. **Bug 修正**：`.emoji-row` 改 grid 後 `display:grid` 蓋掉 `hidden` 屬性 → 補 `.emoji-row[hidden]{display:none}`；加入圖片頭像室友時 toast 印出路徑 → `addMember` toast 套 `itemIconText`
...
- 為了讓已安裝／曾造訪的使用者拿到新圖：index.html 與 manifest 的圖示 URL 全部加 `?v=2` 破快取（下次再換圖記得升 `?v=3`），sw.js `CACHE` v11→v12
- ⚠️ iOS 已加入主畫面的捷徑**圖示不會自動更新**，要刪掉捷徑重新「加入主畫面」才會換新圖；新安裝的直接是新圖
...
- **趣味核心可用 emoji 或使用者的手繪/插圖素材**：室友動物頭像、標題🥚、toast、副標(👇/✨)仍是 emoji；空狀態/示範/結算卡/售罄已換圖。素材放 `public/Assets/`，區塊圖套 `.empty-gif`（200px）、行內小圖套 `.inline-img`（1.3em）、物品圖示套 `.icon-img`（1.15em，跟字級走）；物品圖示的 png 收在 `public/Assets/shared_items/`
- **全站主色是陽光橘**（`--grad` / `--lime`）；漸層按鈕一律白字、hero 卡文字用深色 ink（清楚）
- ⚠️ 慣例更新：**FAB 已從深色改成主漸層**（v1.1 時是深色）。目前深色點綴只剩右上「未結 $XXX」徽章。**選取狀態（emoji/圖示/chip/頭像分頁）一律主漸層框**（v1.5 起，黑框已淘汰，做法見 v1.5 第 1 點）
...
| `public/Assets/` | 裝飾圖：`welcome`／`empty`／`groceries`（jpg 區塊圖）、`outofstock.gif`（售罄動圖）、`Eye`／`Smile`（行內小圖）；`shared_items/*.png` 物品圖示、`avatar_image/avatar_01–08.png` 人物頭像。從根目錄提供，路徑 `public/Assets/xxx` |
